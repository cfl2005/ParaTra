...
def beam_search(model, src, src_mask, max_len, pad, bos, eos, beam_size, device, encode=1):
...
def beam_search_decode(model_decode, model_generator, src, src_mask, max_len, pad, bos, eos, beam_size, device):
    """ Translation work in one batch """
    ...
    def beam_decode_step(
        ...
        def predict_word(dec_seq, enc_output, n_active_inst, n_bm):
            ...
            out = model_decode(enc_output, src_mask,
                                dec_seq,
                                subsequent_mask(dec_seq.size(1))
                                .type_as(src.data))
            word_logprob = model_generator(out[:, -1])
            word_logprob = word_logprob.view(n_active_inst, n_bm, -1)
            # NEW
