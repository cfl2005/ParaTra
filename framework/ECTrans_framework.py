...
# server:  client ==> port(REP) ==> port_task(PUSH)
...
class ECTrans_Client():
    ...
    def send(self, datafile):
        if self.encoder is None:
            raise ValueError("use set_encode ")
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='ECTrans framework')
    ...
    parser.add_argument('--realtime_pipelines', type=int, default=2, help='num of pipeline, min=1')
    parser.add_argument('--batch_pipelines', type=int, default=2, help='the num of batch pipeline ')
    parser.add_argument('--datafile', type=str, default="report/data_100.txt", help='client send data')
